...
`pairs(pigs.emmGrid.s, reverse = TRUE)`. Also, in multi-factor situations,
...
to the `org.emmGrid` call above, would the Bonferroni adjustment be applied to the
...
(or `pairs()`) notices that the response is on the log scale, it back-transforms contrasts to ratios when results are to be of `response` type. For example:
```{r}
