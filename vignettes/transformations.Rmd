...
They are back-transformed from `pigs.emmGrid.s`, in which *the marginal averaging
...
```
will compute the results we have seen for `pigs.emmGrid.s` -- back-transformed 
...
logits are logs of odds; so thias is another instance where log-differences are
back-transformed -- in this case to odds ratios:
```{r}
pairs(neuralgia.emmGrid, reverse = TRUE)
```
So there is evidence of considereably more pain being reported with placebo 
...
Observe that there is a note in the output for `neuralgia.emmGrid` that the results
...
The `make.tran()` function provides several spcial transformations and sets
...
The `regrid()` dunction makes it possible to fake a log transformation of the response. Why would you want to do this? So that you can make comparisons using
