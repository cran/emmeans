...
offer `mode = "boot-satterthwaite"` as a backup, by which quantities related to
`Var(s^2)` are obtained by randomly perturbing the response values. Currently,
only `"satterthwaite"` is unavailable for `lme` objects, and it is used
...
as a factor in the refernce grid, and the arguments `mult.levels` can be used to
