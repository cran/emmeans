...
|rq,rqs       |quantreg |[Q](#Q)  |Creates pseudo-factor `tau` with levels `object$tau`         |
...
The elements
of `object$tau` are included in the reference grid as a pseudo-factor named
...
*Note:* Older versions of `this`rq` and `rqs` support required an argument `tau` 
which is now deprecated and ignored if specified. If you want a particular
subset of the `tau` values, use the `at` argument to `ref_grid`.
