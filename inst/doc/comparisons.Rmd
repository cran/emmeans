...
*Note:* The **multcomp** package provides a very similar function named `cld()`. A
corresponding method is provided for `emmGrid` objects; thus, if `package:multcomp`
is attached, you may call `cld()` and obtain the same results as with `CLD()`.
