...
When an `emmGrid` object is summarized and displayed, the factory default
is to display it in a consistent way with many other analyses in R --- i.e.,
using the number of digits in `getOption("digits")`. This is often much more
precision than is really justified. You may use the
`"opt.digits"` option to change this. If it is set to `TRUE`, the number
of digits is set to a more realistic value based on the estimates and their 
confidence or HPD intervals. In the remainder of this vignette, we turn on 
this option; observe that fewer digits are displayed than those that came before.
```{r}
emm_options(opt.digits = TRUE)
```
By the way, setting this option does *not* round the calculated values computed
by `summary.emmGrid()` or saved in a `summary)emm` object; it simply controls
the precision displayed by `print.summary_emm()`.
...
if you usually want optimal precision and confidence intervals for contrasts,
