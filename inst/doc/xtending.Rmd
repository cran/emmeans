...
This vignette explains how developers may incorporate **emmeans** support in their packages.
  ...
  8. [Exporting your methods](#exporting)
  9. [Conclusions](#concl)
...
For package developers' convenience, **emmeans** exports some of its S3 methods for `recover_data` and/or `emm_basis`---use `methods("recover_data")` and `methods("emm_basis")` to discover which ones. It may be that all you need is to invoke one of those methods and perhaps make some small changes---especially if your model-fitting algorithm makes heavy use of an existing model type supported by **emmeans**. Contact me if you need **emmeans** to export some additional methods for your use.
...
A few additional functions are exported because they may be useful to developers. They are as follows:
  ...
  * `.std.link.labels(fam, misc)` This is useful in `emm_basis` methods for
...
## Exporting your methods {#exporting}
...
### Exporting as functions (recommended)
<!-- @index Exporting extensions to **emmeans** -->
I believe the best way to do this is to simply export the functions themselves; so if
the object class is `"foo"`, the `NAMESPACE` file should contain
```r
export("recover_data.foo", "emm_basis.foo")
```
If you are using **devtools** (or **roxygen2**) to generate this file, give the function 
name explicitly (e.g., `@export recover_data.foo`). 

In addition, the package-checking procedure will insist that you document these
functions. It is enough to simply include these functions as aliases in some
`.Rd` file. You are not actually required to show the syntax of the call in an `.Rd`
file; but if you do, it is permissible to use the construct
`method{emm_basis}{foo}(...)` (or `@method emm_basis foo` in **roxygen2**), even
though you are exporting it as a function.

You should also include **emmeans** in the `Suggests` list in the `DESCRIPTION` file,
and precede any calls to functions therein with `emmeans::`.

### Registering as S3 methods
<!-- @index Registering methods -->
Instead of exporting the functions themselves, you do have the option of
officially registering the methods. It is done like this in the `NAMESPACE` file:
```r
...
```
With the `requireNamespace()` construct, this only has an effect if the user has the **emmeans** package installed (in which case **estimability** is also imported by **emmeans**); otherwise the code is skipped. We import the prototypes for `recover_data` and `emm_basis`, and register our new methods.
Also, packages **emmeans** and **estimability** must be included in the `Imports` section of the `DESCRIPTION` file.

The above approach seems like it *should* be the recommended way. But the reason I
suggest not doing it this way is because it creates unneeded package
dependencies. Recent updates to **emmeans** have included serious efforts to
reduce the number of dependent packages, but still, the dependencies for
**emmeans** will be added to the list of dependencies for your own package, and
they can start to add up. When a package has a lot of dependencies, then there is
a higher likelihood that changes in some other package will create problems,
and it is best to avoid this kind of vulnerability. In my opinion, there needs to 
be a better system for making generics available to other packages without
requiring importing the whole namespace and creating exploding dependencies.
