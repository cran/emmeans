...
Thus, registering the methods must be done conditionally.

One solution is to list `emmeans` and `estimability` in the `Imports` field of your `DESCRIPTION` file.
Then, in the package `NAMESPACE` file, insert something like the following,
replacing "foo" with your object class name:
```r
...
```
(You can get by without testing **estimability** since it is already required by **emmeans**.)

The disadvantage of the above solution is that it adds several other packages as
dependencies. An alternative method that avoids those extra dependencies is as follows:
Put `emmeans` and
`estimability` in `Suggests` rather than `Imports`, and add code to
*dynamically* register the methods to one of your R source files (typically one
named `zzz.R`, the last source file to be read). Put code like this there:
```r
...
```
This code is adapted and simplified from the **hms** package. The original
`register_s3_method` function is more general and includes type-checking
