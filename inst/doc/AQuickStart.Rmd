...
vignette: >
  %\VignetteIndexEntry{Quick-start guide for emmeans}
...
## Contents {#contents}
  ...
  7. [Common things that can go wrong](#problems)
  8. [Further reading](#more)
...
Much of what you do with the **emmeans** package involves these three basic steps:
  ...
  0. Fit a *good* model to your data, and do reasonable checks to make sure
     ...
     If you're not sure whether your model is any good, this is a good times
...
Sometimes you may want to examine *interaction contrasts*, which are contrasts of contrasts. The thing to know here is that `contrast()` or (`pairs()`) creates the same kind of object as `emmeans()`, so you can run them multiple time. For example,
```r
